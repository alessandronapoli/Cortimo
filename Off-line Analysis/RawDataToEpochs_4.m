function [progress, is2D] = RawDataToEpochs_4(DiscCont, ContinuousLabelVR, ContinuousLabel2D, ...
    DiscLabelsHand, DiscLabelsElbow)
...
%             DerivedLabels(DerivedLabels == -1) = 2;
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if is2D == 1
            ...
            if any(newTraining.Trial)
                trialInds = find(newTraining.Trial > 0.1);
            else
                trialInds = 1:height(newTraining);
            end
...
%             DerivedLabels(DerivedLabels == -1) = 2;
        ...
        for i=1:length(trialIndCell)
            ...
            rawtimeInds = zeros(size(A));
            stop = length(A)-mod(length(A),100);
            for j=1:100:stop
                chunk = j:j+100;
                [~,rawtimeInds(chunk,1)] = find(fTime == A(chunk,1));
            end
            [~,rawtimeInds(stop+1:end,1)] = find(fTime == A(stop+1:end,1));
            %   ContVolts{i,1} = rawData(:,rawtimeInds(1):rawtimeInds(end));
