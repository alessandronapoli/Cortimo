function [VRTrainingLabels] = loadVrTrainingFile(filenameT)
...
while(~isempty(temp_4))
   ...
   matrix = reshape(temp_4(step+4:step+datlen-3),stream_length,[])';
   DecOutput = temp_4(step+datlen-2:step+datlen-1)';
...
%    end   
  thismatrix = [thismatrix; [cycle.*ones(size(matrix,1),1),...
       ...
       AbsTime2.*ones(size(matrix,1),1), matrix,DecOutput.*ones(size(matrix,1),2)]];
...
stream_labels = [{'MatlabCycle','MatTime','NSPTime'},stream_labels,{'WriDecOut','ElbDecOut'}];
