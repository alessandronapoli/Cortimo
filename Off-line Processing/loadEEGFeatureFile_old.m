function [EEGfeats] = loadEEGFeatureFile_old(filenameT)
...
while(~isempty(temp_4))
    ...
    thisid = thisid+cn+1;
    Fs = temp_4(thisid);
    freqnum = temp_4(thisid+1);

    thisid = thisid+2;
    EEGfeats{count,1} = {'CortimoCycles','NSPTimeStamps','TimeWin','Fs'};
    EEGfeats{count,2} =  [cycle,NSPTime,TimeWin,Fs];
    EEGfeats{count,3} = ['Ch Labels     ', num2str(channels')];
    % EEGfeats{count,4} = ['Freq   ', num2str(Fstart:Fstep:Fstop-1)];
    EEGfeats{count,4} = ['Freq   ', num2str(0:3:Fs/2-1)];
